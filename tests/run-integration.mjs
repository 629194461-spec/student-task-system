...
try {
  await waitForServer();
  await run(process.execPath, ['--test', 'tests/task-workflow.test.mjs'], { env: { ...process.env, TEST_BASE_URL: base } });
  await run(process.execPath, ['--test', 'tests/task-templates.test.mjs', 'tests/account-system.test.mjs'], { env: { ...process.env, TEST_BASE_URL: base } });
} finally {
