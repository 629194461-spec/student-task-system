...
function renderReviews() {
  const totalPending = state.reviews.length + state.rewardApplications.length;
  ...
  const items = [...state.reviews.map(task => ({ type: 'task', value: task, time: task.submittedAt })), ...state.rewardApplications.map(item => ({ type: 'reward', value: item, time: item.createdAt }))].sort((a, b) => String(b.time || '').localeCompare(String(a.time || '')));
  $('#review-list').innerHTML = items.length ? `<section class="review-student-group"><div>${items.map(item => item.type === 'task' ? reviewListRow(item.value) : rewardReviewListRow(item.value)).join('')}</div></section>` : emptyState('badge-check', '暂时没有待审核事项', '学生提交任务或奖励申请后会显示在这里。');
  refreshIcons();
}
async function loadReviews() {
  const [data, rewardData] = await Promise.all([api('/api/parent/reviews?studentId=all&period=all'), api('/api/parent/reward-applications?status=pending')]); state.reviews = data.reviews; state.rewardApplications = rewardData.applications; renderReviews(); renderRewardReviews();
}
